...
import 'package:lucasbeatsfederacao/models/user_model.dart'; // Importar User
...
class _QRRDetailScreenState extends State<QRRDetailScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final canJoin = currentUser != null && _qrr.canUserJoin(currentUser); // Passando o objeto User
      ...
      child: Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            const SizedBox(height: 16),
            // Exibir os primeiros participantes
            if (_qrr.participants.isEmpty)
              Text(
              ...
              Wrap(
                spacing: 8.0,
                runSpacing: 8.0,
                children: _qrr.participants.take(5).map((participant) => Chip(
                  avatar: CircleAvatar(
                    backgroundColor: Colors.blueGrey,
                    child: Text(participant.username[0].toUpperCase(), style: const TextStyle(color: Colors.white)),
                  ),
                  label: Text(participant.username, style: const TextStyle(color: Colors.white)),
  ...
  Widget _buildActionButton(bool isParticipant, bool canJoin) {
    ...
    if (currentUser == null) {
      return const SizedBox.shrink(); // Não mostra botão se não estiver logado
    }
